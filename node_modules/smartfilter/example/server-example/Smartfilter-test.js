        ...
        */
        mysmartfilter.smartfilterRequest({ type: "pivot", data: { reference: 'myPivot', dimensions: ['type'], measures: [{ key: 'volume', aggregation: 'sum'}]} }, function (output) {
            if (output.type !== 'error') {
                ...
                */
                mysmartfilter.smartfilterRequest({ type: "filter", data: {reference:'First Query', field: 'Qtr', filters: ['Q1'], filterType: 'in'} }, function (output) {
                    if (output.type !== 'error') {
                        console.log(JSON.stringify(output, null, 4), '\n\n');
                        /*
                        ...
                        */
                        mysmartfilter.smartfilterRequest({ type: "filter", data: { field: 'Qtr', filters: ['Q1', 'Q2'], filterType: 'in'} }, function (output) {
                            if (output.type !== 'error') {
                                ...
                                */
                                mysmartfilter.smartfilterRequest({ type: "filter", data: { reference: 'Last Query', field: 'Qtr', filters: ['Q2'], filterType: 'in'} }, function (output) {
                                    if (output.type !== 'error') {
                                        console.log(JSON.stringify(output, null, 4), '\n\n');
                                    }
                                    else {
